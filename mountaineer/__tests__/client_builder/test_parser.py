...
class TestInheritanceHandling:
    ...
    def test_multiple_inheritance_fields(self, parser: ControllerParser):
        ...
        left_wrapper = next(
            sc for sc in wrapper.superclasses if sc.model == LeftInheritanceModel
        )
        right_wrapper = next(
            sc for sc in wrapper.superclasses if sc.model == RightInheritanceModel
        )
    ...
    def test_typescript_generation_for_multiple_inheritance(
        ...
        assert (
            f"interface {DiamondInheritanceModel.__name__} extends {LeftInheritanceModel.__name__}, {RightInheritanceModel.__name__}"
            in ts_code
        )
