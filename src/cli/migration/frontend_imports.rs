...
pub(super) struct FrontendImportMigration;

impl Migration for FrontendImportMigration {
    fn name(&self) -> &'static str {
        "frontend imports"
    }

    fn description(&self) -> &'static str {
        "replace \"./_server\" with \"./.mountaineer\""
    }
...
"#,
        ...
        let migration = FrontendImportMigration;
        let files = migration.scan(root.path()).unwrap();
        assert_eq!(files.len(), 1);
        migration.apply(&files).unwrap();
