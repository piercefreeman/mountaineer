...
def install_poetry():
    ...
    try:
        with NamedTemporaryFile(delete=False) as tmp_file:
            subprocess.run(["curl", "-sSL", "https://install.python-poetry.org", "-o", tmp_file.name], check=True)
...
def get_git_user_info():
    ...
    """
    try:
        git_user_name = subprocess.check_output(["git", "config", "--global", "user.name"]).strip().decode()
        git_user_email = subprocess.check_output(["git", "config", "--global", "user.email"]).strip().decode()
