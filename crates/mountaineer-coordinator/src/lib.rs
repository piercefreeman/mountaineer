...
impl ViteDevServer {
    async fn spawn(config: &CoordinatorConfig, directory: &TempDir) -> Result<Self> {
        let package_json = config.frontend_root.join("package.json");
        let vite_entrypoint = config
            .frontend_root
            .join("node_modules")
            .join("vite")
            .join("bin")
            .join("vite.js");
        if !vite_entrypoint.is_file() {
            return Err(invalid(format!(
                "Vite is not installed under {}. Run `npm install -D vite @vitejs/plugin-react`.",
                config.frontend_root.display()
            )));
        }
        ...
        let mut child = Command::new("node")
            ...
            .env("MOUNTAINEER_PACKAGE_JSON", package_json)
