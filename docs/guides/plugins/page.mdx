...
| Plugin | What you get |
| --- | --- |
| [Auth](/guides/plugins/auth) | Email/password auth, sessions, password reset, admin users |
| [Billing](/guides/plugins/billing) | Stripe subscriptions, metered usage, webhooks, checkout |
| [Cloud](/guides/plugins/cloud) | File storage and email across AWS, Cloudflare, DigitalOcean, and Resend |
...
│       │       └── .mountaineer/    # Generated server hooks
│       └── .mountaineer/static/     # Compiled assets
└── README.md
```
...
- `view_root`: Path to the plugin's view directory. Required when plugin controllers use relative `view_path` values. If every controller already uses a `ManagedViewPath` under one shared root, Mountaineer can infer it.
- `router`: Optional FastAPI `APIRouter` for plugin-owned API endpoints that are not tied to a controller action
...
```toml title="pyproject.toml"
[tool.hatch.build]
artifacts = ["your_plugin/views/.mountaineer"]
...
- **[mountaineer-auth](/guides/plugins/auth)**: Login, signup, sessions, and password reset with pre-built views
- **[mountaineer-billing](/guides/plugins/billing)**: Stripe subscriptions, metered usage, and webhook handling
- **[mountaineer-cloud](/guides/plugins/cloud)**: File storage and email across AWS, Cloudflare, DigitalOcean, and Resend
