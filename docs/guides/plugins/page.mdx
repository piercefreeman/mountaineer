...
- `router`: FastAPI `APIRouter` for plugin-owned API endpoints that are not tied to a controller action

If you prefer to declare endpoints directly on the plugin instance, `MountaineerPlugin` also exposes a default router:
...
@plugin.router.get("/your-plugin/api/health")
def healthcheck():
    return {"status": "ok"}
```
