...
```typescript
import { HTTPValidationErrorException } from "./.mountaineer";
...
Mountaineer converts the error into a custom class and exports it from the view's `.mountaineer` module. This class helps you switch logic depending on the type of error that was raised. Using a class here also has the benefit of typeguarding your error handling, so you'll see IDE recommendations specific to that ValidationError.
...
```typescript title=".mountaineer/actions.ts"
