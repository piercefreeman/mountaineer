...
# Node.js runs Vite while Mountaineer builds the browser and SSR artifacts.
RUN apt-get update \
...
The final image does not include Node.js or Vite. Production serves Vite's
browser artifacts as static files and executes its self-contained SSR artifacts
inside Mountaineer's embedded V8 runtime.
