...
Streaming responses are a useful paradigm in webapp design, and increasingly utilized to provide realtime Server->Client data for use in user notifications, CI feedback, and model inference.
...
For the time being we only support streaming responses in `@passthrough` functions, not `@sideeffect`. It's ill-defined whether we should re-render() content every yield or when the iterator has finished. Yielding in passthrough makes it clear that you just want to stream the yielded value to the client.
