...
def update_version_rust(new_version: str):
    cargo_paths = [
        ...
        for path in [
            Path("Cargo.toml"),
            Path("crates/mountaineer-coordinator/Cargo.toml"),
        ]
        if path.exists()
    ]
    if not cargo_paths:
    ...
    for cargo_path in cargo_paths:
        ...
        if "package" not in filedata:
            raise ValueError(f"{cargo_path} is missing the [package] section")
...
def update_version_python(new_version: str):
    pyproject_paths = [
        ...
        for path in [
            Path("pyproject.toml"),
            Path("crates/mountaineer-coordinator/pyproject.toml"),
        ]
        if path.exists()
    ]
    ...
    for pyproject_path in pyproject_paths:
        ...
        if "project" not in filedata or "version" not in filedata["project"]:
            print(  # noqa: T201
                f"Warning: [project.version] not found in {pyproject_path}"
            )
            continue
        ...
        if (
            ...
            and "dependencies" in filedata["project"]
        ):
            filedata["project"]["dependencies"] = [
                f"mountaineer-coordinator=={python_version}"
                if dependency.startswith("mountaineer-coordinator==")
                else dependency
                for dependency in filedata["project"].get("dependencies", [])
            ]
